...
DwarfVariableFinder::DwarfVariableFinder(StringRef Filename) {
  ...
  OS = new  std::fstream (outFile, std::ios::out | std::ios::trunc | std::ios::binary);
  error(outFile, EC);
}
...
/* 
 * Purpose: Find all the variables (locals & gobals) inside the compilation unit die 'die'
 */
...
/*
 * Purpose: Find all the variables in the scope die. scope die could be a subroutine
 *          or compilation unit die.  
 */
void DwarfVariableFinder::findVariablesInScope(const DWARFDie &scope_die) {
  for (auto child = scope_die.getFirstChild(); child; child = child.getSibling()) {
    switch(child.getTag()) {
      ...
      case dwarf::DW_TAG_constant:
      {
        ...
        scope->set_symbol_name(dwarf::toString(scope_die.find(dwarf::DW_AT_name), "None"));


        LV->set_name(dwarf::toString(child.find(dwarf::DW_AT_name), "None"));

        auto *TY = LV->mutable_type();
        auto type = getType(child.getAttributeValueAsReferencedDie(dwarf::DW_AT_type), TY);
...
std::shared_ptr<::VariableType::Type> DwarfVariableFinder::getType(const DWARFDie &die, 
    ::VariableType::Type *TY) {
...
std::shared_ptr<::VariableType::Type> DwarfVariableFinder::makeType(const DWARFDie &die, 
    ::VariableType::Type *TY) {
  ...
  switch (die.getTag()) {
    ...
    case dwarf::DW_TAG_pointer_type: 
    {
      ...
      auto baseType = getType(die.getAttributeValueAsReferencedDie(dwarf::DW_AT_type), ETY);
    ...
    case dwarf::DW_TAG_union_type: 
    {
      ...
      TY->set_c_type(std::string("struct") + dwarf::toString(die.find(dwarf::DW_AT_name), "None"));
      ...
      for (auto childDie = die.getFirstChild(); childDie && childDie.getTag(); childDie = childDie.getSibling()) {
        if (childDie.getTag() != dwarf::DW_TAG_inheritance && childDie.getTag() != dwarf::DW_TAG_member) {
          continue;
        }
        auto *field = TY->add_member_list();
        field->set_field_offset(dwarf::toUnsigned(childDie.find(dwarf::DW_AT_data_member_location), ~0U));
        field->set_field_name(dwarf::toString(childDie.find(dwarf::DW_AT_name), "None"));
